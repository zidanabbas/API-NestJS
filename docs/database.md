...
| `role`                    | `UserRole` (`ADMIN` \| `CUSTOMER`) | Default `CUSTOMER`. Ikut disisipkan ke payload JWT saat login dan ditegakkan oleh `RolesGuard` + `@Roles(UserRole.ADMIN)` pada endpoint tulis menus/categories/tables serta update status order & payment |
...
| `quantity` | `Int`                   |                                           |
| `price`    | `Decimal(12,2)`         | Harga menu saat pesanan dibuat (snapshot) |
...
- **2026-08-18** — model `Product` di rename menjadi `Menu` (tabel `Product` → `Menu`, kolom `OrderItem.productId` → `menuId`), lewat migration [`20260818120000_rename_product_to_menu`](../prisma/migrations/20260818120000_rename_product_to_menu/migration.sql).
...
Module **`payments` sudah diimplementasikan** — buat pembayaran QRIS untuk sebuah order (`POST /api/v1/payments`), cek status, dan konfirmasi status oleh ADMIN (`PATCH /api/v1/payments/:id/status`). Lihat [features/payments.md](features/payments.md).

Status order pun sudah bisa diubah lewat **`PATCH /api/v1/orders/:id/status`** dengan validasi state machine. Lihat [features/orders.md](features/orders.md#patch-apiv1ordersidstatus--admin--ubah-status-pesanan).

Yang **masih menjadi pengembangan berikutnya**:

- Filter/pagination untuk `GET /api/v1/orders` dan `GET /api/v1/tables`.
- **Integrasi gateway QRIS asli** — `qrString`/`qrUrl` saat ini masih placeholder; perlu webhook untuk update `PaymentStatus` otomatis dan pengisian `transactionId`.
- **Payment `PAID` belum otomatis mengubah `order.status`** — idealnya digabung dalam satu transaksi.
...
> **Koneksi untuk migrasi.** Proyek ini memakai Supabase dengan dua connection string (lihat [`.env`](../.env)):
>
